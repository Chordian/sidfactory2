...
namespace Converter
{
	SourceMod::SourceMod(SF2::Interface* inInterface, Misc* inOutput, unsigned char* inByteBuffer)
		: m_SF2(inInterface)
		, m_Output(inOutput)
    ...
    void SourceMod::Unsupported(const std::string& inModCommand)
    {
        ...
        std::cerr << "WARNING: MOD command " << inModCommand << " is not supported by the converter." << std::endl;
	 ...
	 */
	void SourceMod::Convert(int inLeaveout, int inTranspose)
	{
		...
		for (int c = 0; c < m_ModMaxPattern * 256; c++)
		{
			...
			if (c % 256 == 0)
			{
                ...
                std::cout << std::setfill('0') << "Pattern #" << std::hex << std::setw(2) << (int)c / 256 << "\n-----------\n";
			}
#endif // DEBUG_OUTPUT
...
#ifdef DEBUG_OUTPUT
			if (c % 4 == 0) std::cout << std::hex << std::setw(2) << (int)(c / 4 & 0x3f) << ":  ";
			if (note) std::cout << c_NotesSharp[(int)(note - 1) % 12] << (int)(note - 1) / 18 << " "; // (Why 18 and not 12?)
			else std::cout << "--- ";
			if (instrument) std::cout << std::hex << std::setw(2) << (int)instrument << " ";
			else std::cout << "-- ";
			if (command || source[3]) std::cout << std::hex << std::setw(1) << (int)command << std::hex << std::setw(2) << (int)source[3];
			else std::cout << "---";
			if (c % 4 == 3) std::cout << std::endl;
			else std::cout << "  ";
#endif // DEBUG_OUTPUT
        ...
        std::cout << "Ignoring MOD channel [" << inLeaveout << "] thus converting ";
        for (int channel = 1; channel <= 4; channel++)
            if (channel != inLeaveout) std::cout << "[" << channel << "]";
        std::cout << " to SF2.\n";
        if (inLeaveout == 4)
            std::cout << "Use the /l:<number> switch if you want to ignore a different channel.\n";

        inTranspose
            ? std::cout << "The notes will be transposed +" << inTranspose << " halfsteps in the SF2 version.\n"
            : std::cout << "The notes will not be transposed; use the /t:<number> switch to do that.\n";
                ...
                for (int orderlist_pos = 0; orderlist_pos < m_ModOrderListLength; orderlist_pos++)
                {
                    ...
                    for(int pos = 0; pos < 64; pos++)
                    {
                        ...
                        if (break_pattern)
                        {
                            ...
                            if (!break_note_shown)
                            {
                                std::cout << "NOTE: The Bxx and Dxx commands only cut their sequence shorter in SF2.\n";
