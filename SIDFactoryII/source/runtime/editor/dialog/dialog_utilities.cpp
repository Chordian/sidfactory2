...
namespace Editor
{
	DialogUtilities::DialogUtilities(int inWidth, int inHeight, std::function<void(const DialogUtilities::Selection)>&& inSelect, std::function<void(void)>&& inCancel)
		: DialogBase()
		, m_Width(inWidth)
		, m_Height(inHeight)
	...
	bool DialogUtilities::ConsumeInput(const Foundation::Keyboard& inKeyboard, const Foundation::Mouse& inMouse)
	{
	...
	void DialogUtilities::ActivateInternal(Foundation::Viewport* inViewport)
	{
		...
		const std::string caption = "Utilities";
		...
		m_StringListDataBuffer = std::make_shared<DataSourceTList<std::string>>(std::vector<std::string>({ "Select song", "Add song", "Remove song", "Statistics", "Optimize", "Pack", "Clear Sequences", "Expand sequences"}));
	...
	void DialogUtilities::DeactivateInternal(Foundation::Viewport* inViewport)
	{
