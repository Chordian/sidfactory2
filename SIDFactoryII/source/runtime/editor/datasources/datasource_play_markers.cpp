...
namespace Editor
{
	DataSourcePlayMarkers::DataSourcePlayMarkers(const AuxilaryDataPlayMarkers& inAuxilaryData, const AuxilaryDataSongs& inAuxilarySongs, const DisplayState& inDisplayState)
		...
		, m_AuxilaryDataSongs(inAuxilarySongs)
	{
	...
	std::string& DataSourcePlayMarkers::operator[](int inIndex)
	{
		...
		const int event_pos = m_AuxilaryDataPlayMarkers.GetPlayMarkerEventPosition(m_AuxilaryDataSongs.GetSelectedSong(), inIndex);
